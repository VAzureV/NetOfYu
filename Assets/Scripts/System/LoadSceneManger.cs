/****************************************************
    文件：LoadSceneManger.cs
	作者：Azure
	功能：切换场景的管理器。用于加载场景和切换场景。
*****************************************************/
...
public class LoadSceneManager : Singleton<LoadSceneManager>
{
    ...
    /// <param name="completed">加载完毕后的回调。</param>
    /// <param name="setActiveAfterCompleted">加载场景完毕后，是否切换到该场景。</param>
    public void LoadSceneAsync(int sceneBuildIndex, LoadSceneParameters parameters, UnityAction<float> loading = null, UnityAction<AsyncOperation> completed = null, bool setActiveAfterCompleted = true)
    {
        //如果要加载的场景的索引不合法，则返回。
        if (!IsSceneBuildIndexValid(sceneBuildIndex))
            return;
        ...
        //开启协程进行异步加载。
        MonoManager.Instance.StartCoroutine(LoadSceneAsyncCoroutine(sceneBuildIndex, parameters, loading, completed, setActiveAfterCompleted));
    }
    IEnumerator LoadSceneAsyncCoroutine(int sceneBuildIndex, LoadSceneParameters parameters, UnityAction<float> loading = null, UnityAction<AsyncOperation> completed = null, bool setActiveAfterCompleted = true)
    {
        //异步操作对象，记录了异步操作的数据。
    ...
    /// <param name="setActiveAfterCompleted">加载场景完毕后，是否切换到该场景。</param>
    /// <param name="mode">加载场景的模式。默认是LoadSceneMode.Single，表示会卸载原来的场景，再切换到新场景。LoadSceneMode.Additive表示会将新场景叠加在原来的场景中。</param>
    public void LoadSceneAsync(string sceneName, LoadSceneParameters parameters, UnityAction<float> loading = null, UnityAction<AsyncOperation> completed = null, bool setActiveAfterCompleted = true)
    {
        //开启协程进行异步加载。
        MonoManager.Instance.StartCoroutine(LoadSceneAsyncCoroutine(sceneName, parameters, loading, completed, setActiveAfterCompleted));
    }
    IEnumerator LoadSceneAsyncCoroutine(string sceneName, LoadSceneParameters parameters, UnityAction<float> loading = null, UnityAction<AsyncOperation> completed = null, bool setActiveAfterCompleted = true)
    {
        //异步操作对象，记录了异步操作的数据。
